...
import WoofiRouter from "../../action/woofiRouter";
...
class BaseWoofiEthToTokenSwap extends Block {
  name = "";
  ...
  async ethToTokenSwap(account: Account) {
    ...
    const woofi = new WoofiRouter();

    const { hash, inReadableAmount, outReadableAmount } = await woofi.swap({
      account,
      fromToken: native,
      toToken: this.token,
  ...
  async approve(account: Account) {
    ...
    const contractAddress = woofi.getAddressToApprove(this.chain);

    if (!contractAddress) {
      throw new Error(
        `Contract address of woofi for ${this.chain} is not found`
      );
    }
...
export default BaseWoofiEthToTokenSwap;
