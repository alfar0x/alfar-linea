...
class BaseVelocoreEthToTokenSwap extends Block {
  ...
  async ethToTokenSwap(account: Account) {
    ...
    const velocore = new VelocoreVault();

    const { hash, inReadableAmount, outReadableAmount } = await velocore.swap({
      account,
      fromToken: native,
      toToken: this.token,
  ...
  async approve(account: Account) {
    ...
    const contractAddress = velocore.getAddressToApprove(this.chain);
...
export default BaseVelocoreEthToTokenSwap;
