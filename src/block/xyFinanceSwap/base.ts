...
class BaseXyFinanceEthToTokenSwap extends Block {
  ...
  async ethToTokenSwap(account: Account) {
    ...
    const xyFinance = new XyFinanceRouter();

    const { hash, inReadableAmount, outReadableAmount } = await xyFinance.swap({
      account,
      fromToken: native,
      toToken: this.token,
  ...
  async approve(account: Account) {
    ...
    const contractAddress = xyFinance.getAddressToApprove(this.chain);

    if (!contractAddress) {
      throw new Error(
        `Contract address of xy finance for ${this.chain} is not found`
      );
    }
...
export default BaseXyFinanceEthToTokenSwap;
