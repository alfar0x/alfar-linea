...
import PancakeRouter from "../../action/pancakeRouter";
...
class BasePancakeEthToTokenSwap extends Block {
  name = "";
  ...
  async ethToTokenSwap(account: Account) {
    ...
    const pancake = new PancakeRouter();

    const { hash, inReadableAmount, outReadableAmount } = await pancake.swap({
      account,
      fromToken: native,
      toToken: this.token,
  ...
  async approve(account: Account) {
    ...
    const contractAddress = pancake.getAddressToApprove(this.chain);

    if (!contractAddress) {
      throw new Error(
        `Contract address of pancake for ${this.chain} is not found`
      );
    }
...
export default BasePancakeEthToTokenSwap;
